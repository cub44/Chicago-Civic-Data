...
[Explore the project](https://connorblandford.com/projects/cps-space-colocation/). This is the dataset behind an exploratory map; the accompanying article is forthcoming. It examines school space and potential co-location. Health-centre operating status is tracked separately. Snapshot: **15 September 2026**; utilization year: **SY2026**.
...
| [sbhc_publish.csv](data/processed/sbhc_publish.csv) | 38 | One school-based health centre |
| [sbhc_resolved.csv](data/processed/sbhc_resolved.csv) | 38 | One centre with source and resolved values side by side |
| [sbhc_discrepancies.csv](data/processed/sbhc_discrepancies.csv) | 151 | One centre-field evidence finding |
...
| [senior_centers.csv](data/processed/senior_centers.csv) | 21 | One DFSS senior centre |
| [senior_center_buildings.csv](data/processed/senior_center_buildings.csv) | 21 | One senior centre, with a coordinate inside its building |
| [workforce_centers.csv](data/processed/workforce_centers.csv) | 5 | One DFSS workforce centre |
| [workforce_center_buildings.csv](data/processed/workforce_center_buildings.csv) | 5 | One workforce centre, with a coordinate inside its building |
...
Inputs are the CPS SY2026 Space Utilization workbook (`spaceuse_2026_final_forweb.xlsx`, published 19 December 2025), the CPS School Profile Information API (`api.cps.edu/schoolprofile/CPS/AllSchoolProfiles`), and City of Chicago datasets `pb6d-zzuh` (school locations), `igwz-8jzy` (community areas), `x8fc-8rcq` (library branch locations, pulled 13 September 2026), `vcti-mbcd` (the Chicago Park District building inventory behind the portal's `u7uu-j2ma` map view, pulled 14 September 2026), `kcki-hnch`, `qhfc-4cw2` and `cs4s-nsna` (the CDPH clinic, senior centre and workforce centre rosters behind the portal's `4msa-kt5t`, `8ayb-6mjs` and `i4rz-w47p` map views, pulled 14 September 2026) and `syp8-uezg` (building footprints, a 2015 snapshot). Seven schools no primary source geocodes are instead placed from a Google Earth address lookup, 15 September 2026 — a hand-researched input, not a portal pull, flagged as such in every row it touches; see "Before reporting" and `data/README.md`. Most source snapshots are preserved privately. The pages cited by the SBHC reconciliation are preserved in `data/raw/2026-09-12/`, and its standard-library Python build is included so the publication sheet remains auditable.
...
- Seven schools had no coordinate from either primary source; all seven are now filled from a Google Earth geocode dated 2026-09-15, flagged as such in `source`. 14 co-located school rows lack classroom and capacity counts. Absence is not zero.
...
of the CDPH health-centre dataset.
