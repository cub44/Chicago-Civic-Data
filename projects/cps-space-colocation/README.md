...
See the [data dictionary](data/README.md) for all fields and caveats. Join schools and utilization using `sid`, schools and school buildings using `sid`, libraries and library buildings using `name`, and the two park files using `objectid` — not `bldg_id`, which is duplicated on two Lincoln Park buildings. Campus totals overlap individual school records: do not add both together. The building files never replace a roster coordinate; they add a second one, inside the building, and say how confidently it was derived. [SHA-256 checksums](checksums.sha256) identify the download versions.
