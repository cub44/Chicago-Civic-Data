...
[Explore the project](https://connorblandford.com/projects/cps-space-colocation/). This is the dataset behind an exploratory map and the article it accompanies, “Chicago’s underutilized schools are an unrealized opportunity” (part one of three). It examines school space and potential co-location. Health-center operating status is tracked separately. Snapshot: **September 21, 2026**; utilization year: **SY2026**.
...
| [sbhc_discrepancies.csv](data/processed/sbhc_discrepancies.csv) | 151 | One center-field evidence finding |
...
Inputs are the CPS SY2026 Space Utilization workbook (`spaceuse_2026_final_forweb.xlsx`, published December 19, 2025), the CPS School Profile Information API (`api.cps.edu/schoolprofile/CPS/AllSchoolProfiles`), and City of Chicago datasets `pb6d-zzuh` (school locations), `igwz-8jzy` (community areas), `x8fc-8rcq` (library branch locations, pulled September 13, 2026), `vcti-mbcd` (the Chicago Park District building inventory behind the portal's `u7uu-j2ma` map view, pulled September 14, 2026), `kcki-hnch`, `qhfc-4cw2` and `cs4s-nsna` (the CDPH clinic, senior center and workforce center rosters behind the portal's `4msa-kt5t`, `8ayb-6mjs` and `i4rz-w47p` map views, pulled September 14, 2026) and `syp8-uezg` (building footprints, a 2015 snapshot). Six schools converted from charters in 2026 are placed from `pb6d-zzuh` under the charter ids they held before; see "Before reporting" and `data/README.md`. Most source snapshots are preserved privately. The pages cited by the SBHC reconciliation are preserved in `data/raw/2026-09-12/`, and its standard-library Python build is included so the publication sheet remains auditable.
